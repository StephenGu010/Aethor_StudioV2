...
- 动作编排：当前仅有阶段 2 的只读信息架构入口；版本化 JSON、点位编辑、单点采集、校验和受控顺序执行属于阶段 6，落地前不得宣称可用。
...
规划中的防御式停止链为：`!STOP → $0,0,0,0,0,0 → !DISABLE → #GETENABLE`。其中 `$0...` 是无成功 ACK 的内部 best-effort 步骤，不能通过 UI/公共 API 构造，也不能阻塞后续去使能与读回。只有最终读回使能为 `0` 才能显示“已去使能”；否则显示未确认并引导使用物理急停。
