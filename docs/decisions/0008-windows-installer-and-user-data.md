...
9. 正式 MSI/bundle 必须携带与便携包同源的第三方 SPDX 2.3 清单、机器可读完整性摘要、模型 NOTICE/provenance、模型再分发状态和法律附件。生产依赖只能从锁定的 pnpm 图及实际发布 `.deps.json` 派生；包根遗漏许可正文时，只能使用同时绑定精确组件版本、包完整性、不可变上游 revision/blob 与双 SHA-256 的仓库输入。`releaseReady` 同时要求依赖正文无缺口且两个模型的再分发条款完整，安装器签名不能覆盖或降级任一门。

## 后果

- 用户数据保留不依赖卸载器脚本中的例外分支，二进制与运行数据所有权清晰。
- 正式安装器实现暂时被工具治理、组织发布者身份、签名证书和离线 WebView2 Runtime 策略阻止；这不影响便携包、DPI 和崩溃恢复的软件验证。
- 当前 92 个生产组件的正文已由包内文件或 6 个精确版本/哈希绑定的上游记录闭合；两个模型的再分发材料仍不完整。签名只能证明发布者和完整性，不能替代模型许可条款。
...
5. Dummy 与 Aethor_robo 模型的完整再分发条款或经批准的书面法律处置；依赖版本变化时须重新验证现有六个许可来源绑定。
