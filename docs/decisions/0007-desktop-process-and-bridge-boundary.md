...
4. `DesktopBridgeV1` 只暴露 `minimize / toggleMaximize / close / beginDrag / exportDiagnostics`。请求严格校验版本、ID、动作和来源；浏览器模式不模拟原生成功。诊断导出仍由桌面壳控制文件选择、日志快照、脱敏和原子写盘，Web 页面不读取任意本地文件。
...
- 当前 118 项桌面单元测试覆盖参数、路径、令牌、桥接、日志与诊断包、窗口恢复、端口、代理隔离和进程启动边界。
- 当前自包含开发包共 697 个文件，其中 manifest 逐文件校验 696 项；disabled 与显式 engineering 两种离线 smoke 证明网关 ready、会话 offline、关闭 202 且进程退出。`Legal/` 集中包含两个 Profile 的 NOTICE/provenance、模型再分发状态，以及 92 个生产组件的 SPDX 2.3 清单和法律附件；6 个依赖正文已由精确版本/哈希绑定的上游记录闭合，两个模型条款继续被机器摘要与 release verifier 明确阻断，不能冒充正式候选。
