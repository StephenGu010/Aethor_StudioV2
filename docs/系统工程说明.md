...
| `/terminal` | 串口终端 | Dummy | TX/RX/错误帧查看、过滤、导出和工程调试输入 |
| `/devices` | 设备与模型 | Dummy、Aethor_robo | Profile、串口、会话、URDF、关节映射和能力信息 |
| `/actions` | 动作编排 | Dummy | 动作程序的离线编辑、导入、导出和目标预览 |

旧路径 `/twin` 只保留到 `/console` 的兼容重定向，不再作为正式路由使用。进入当前 Profile 不支持的工作区时，`ProfileWorkspaceGate` 显示能力说明和切换入口，不会把 Dummy 的页面行为套用到 Aethor_robo。
...
- 连接首轮查询顺序为 `#GETJPOS`、`#GETMODE`、`#GETENABLE`；之后 `#GETJPOS` 默认每 50 ms 请求，模式和使能默认每 500 ms 刷新；滑条、反馈、动作点位和命令统一保存设备角，只有 3D 渲染按 Profile 换算模型角（Dummy J3 为 `model=device-90°`）；
...
具体命令、回包和完成语义见 [Dummy ASCII v1](protocols/dummy-ascii-v1.md)。Aethor_robo 尚无可执行串口协议，后续不能直接复用 Dummy formatter。
...
两种轮询节拍并不代表两条串口线程。所有查询继续串行通过同一个 `serialIoGate`，不会并发读取或创建第二个端口 owner。首次连接和任何超时恢复都必须重新完成一次位置、模式、使能的完整状态周期；其中任一项失败时，页面保持 stale，不把局部成功解释为设备可控。
...
`accepted` 或 FIFO 数字只表示网关或固件队列接收。需要物理完成证据的命令由网关继续读取关节反馈并按完成策略收束。当前结构化关节组能力只有在速度、到位容差、稳定窗口和总超时四项配置完整时才会对 UI 开放。

开发模式另有受限 direct command，但仍经过 C# 白名单、会话状态、限位、速度和串口所有权检查；它不是浏览器直接写串口的旁路。
