...
| `aethor-robo-dual-7dof` | Aethor_robo | 空间机器人，左、右机械臂各 7 轴 | 整机模型和 14 轴本地 FK 预览已接入；六个车轮只参与模型展示；固件协议与硬件网关待后续实现 |
...
Application 层已经完成 `SerialDuplexScheduler` 软件门：它对一个已打开 transport 建立唯一持续 reader、唯一 writer、RX 有界队列和 P0–P3 有界发送队列。非安全工作不能占用安全预留槽；满载时安全工作可以淘汰最早的低优先级项；每项工作有排队时效和独立完成 ticket。读取、写入或 handler 故障会停止 session 并关闭 transport，避免不可取消的原生 I/O 阻塞退出。探针可读取队列深度、接受/拒绝/写入/过期/淘汰/失败计数和最后故障，不包含 payload。

该调度器目前只通过 fake transport 验证，尚未注册生产 DI。Dummy 仍由 `serialIoGate` 执行旧问答循环；在 A1-U2 一次性迁移所有读写并删除旧 reader 前，不能并行启动新调度器。Aethor 固件与 CRC vectors 冻结后再添加独立 codec 和 request registry。
...
两种轮询节拍并不代表两条串口线程。所有查询和命令继续串行通过同一个 `serialIoGate`，不会并发读取或创建第二个端口 owner。位置周期按起点计时，串口往返耗时不会在周期后再次叠加；命令声明进入后，尚未获得串口的轮询让行。首次连接和任何超时恢复都必须重新完成一次位置、模式、使能的完整状态周期；其中任一项失败时，页面保持 stale，不把局部成功解释为设备可控。

### 9.5 命令流程

结构化命令从 UI 产生 DTO，经前端能力和状态检查后提交 REST。C# 网关再次验证 Profile、session、命令参数和当前设备状态，然后通过唯一串口所有权写入固件。命令结果进入审计历史，并通过 HTTP 响应与 SignalR 变化通知返回前端。
...
开发模式另有受限 direct command，但仍经过 C# 白名单、会话状态、限位、速度和串口所有权检查；它不是浏览器直接写串口的旁路。六轴运动在 transport 写入完成后返回 `SENT · MANUAL CONFIRM` 并立即允许下一次人工目标，不等待 FIFO、最终 `ok` 或到位。迟到回包只保留为日志，`#GETJPOS` 暂停时页面保留最后值并标为 stale。即使 `#GETJPOS` 持续回包，写入后至少 500 ms、至少 8 帧完全不动且仍远离目标时，网关也会把关节反馈降为 stale；任一关节重新变化后自动恢复。该判断只说明固件公布的角度没有进展，不能判断实机是否运动；这套人工确认语义不适用于正式结构化动作执行。
...
Aethor_robo 应实现独立 adapter，例如后续版本化的 `aethor-robo-*-v1`。双七轴的分组命令、反馈顺序、停止、到位和错误语义应来自其固件，而不是把 Dummy 六轴 DTO 扩成 14 个数字后直接复用。
...
| 实时示波与终端 | Dummy 软件门已实现；Aethor 共用终端外壳已实现 | Aethor 仅候选校验，真实 adapter/串口/TX 不存在；Dummy 双工迁移未完成 |
| Aethor_robo 模型与双七轴预览 | 已实现 | 整机、左右臂取景和 14 轴本地 FK 可用 |
| 共用串口双工运行时 | 软件门已实现 | 未注册生产 DI；Dummy A1-U2 与 Aethor A1-H 分别接线 |
| Aethor_robo 固件网关 | 待实现 | adapter 当前为 `aethor-robo-pending` |
