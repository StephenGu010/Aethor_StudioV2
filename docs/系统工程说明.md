...
|---|---|---:|---|
| `dummy-6dof` | Dummy | 单机械臂，6 轴 | 模型、串口枚举、会话、反馈、终端、结构化命令和 engineering 动作编排已接入 |
...
| `/actions` | 动作编排 | Dummy | 手动示教采点、自动保存、导入导出、目标预览和 engineering 单次/循环运行 |
...
开发模式另有受限 direct command，但仍经过 C# 白名单、会话状态、有限六轴数值、速度和串口所有权检查；它不应用旧 Profile/URDF 角度范围，也不是浏览器直接写串口的旁路。HTTP 入队后返回 `QUEUED · GATEWAY ACCEPTED`，物理写入后再发布 `SENT · MANUAL CONFIRM`；两阶段都不等待 FIFO、最终 `ok` 或到位，也不阻止下一次人工目标。迟到回包只保留为日志，`#GETJPOS` 暂停时页面保留最后值并标为 stale。即使 `#GETJPOS` 持续回包，写入后至少 500 ms、至少 8 帧完全不动且仍远离目标时，网关也会把关节反馈降为 stale；任一关节重新变化后自动恢复。该判断只说明固件公布的角度没有进展，不能判断实机是否运动。

动作编排复用同一 direct 写入证据，但增加独立的 C# 生命周期 owner。前端把自动保存后的 authored revision 转换为不可变运行快照；`EngineeringActionProgramRuntime` 逐点等待 `sent + transportWritten`，按最大关节角差/程序速度加点位附加等待推进，并在 C# 内完成循环。单次结束为 `finishedUnconfirmed`，循环持续到操作员停止。停止会取消未来点位并尝试 `!STOP`、`!DISABLE`；结构化停止、终端停止、断开和桌面关闭都经过同一取消路径。前端只显示权威快照，不自行遍历点位或维护第二个串口队列。
...
| 动作编辑与动作库 | `useActionProgramStore` | 当前编辑停止 350 ms 后自动校验并写入本地持久层；选择与预览标记只属于当前会话 |
| Engineering 动作运行 | C# `EngineeringActionProgramRuntime` | 当前 Dummy session；单一不可变快照，运行终态后保留最新快照，session 改变时前端清空 |
...
串口断开被视为一个新会话边界。连接状态、反馈、命令历史、临时目标和遥测回到初始状态；已经自动保存到动作库的文档、导出文件、布局和显示偏好不随串口断开删除。
...
| Dummy 动作编排 | engineering 单次/循环运行已接线 | transport-written 人工完成模型可用；反馈确认式监督执行仍未接线 |
