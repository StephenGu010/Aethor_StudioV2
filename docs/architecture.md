...
仓库已经包含可运行的 React/Vite 前端、共享 TypeScript/JSON Schema 契约、Dummy 与 Aethor_robo 两个内置 Profile、Dummy 专属 .NET 10 网关，以及 Phase 8A 的 WinForms/WebView2 桌面壳。Aethor_robo A0 模型与双七轴本地控制台、A1-U0 候选电机帧与 ID 诊断、A1-U1/U2 有界双工基础、A1-T0 数字孪生实时内核、A1-H0 主机协议 codec 和 A1-H1-S 未注册会话软件核心已完成；Aethor 会话仍未进入生产 DI、REST/SignalR、动作执行或真实硬件状态层。Phase 5 Gate B 运动未执行；Phase 6A 已实现 Dummy 离线动作编辑器，6B-S 已实现无生产接线的 C# 执行内核，6B-H 硬件接线未开始。Phase 7A 已实现 Dummy 有界实时示波/协议观测，7B 真实网关长测未开始；Phase 8B 的安装签名、DPI 与正式发布门尚未完成。当前代码统一为：
...
- `/actions` 是 Dummy 动作编辑与 engineering 人工运行工作台：允许新建、导入、自动保存、点位编辑/排序、`#GETJPOS` 手动示教采点和目标草稿预览。只有当前网关协商为 engineering、Dummy 会话/六轴反馈新鲜有效、设备已使能且点位模式一致时，运行入口才开放。
...
Aethor_robo console ──> profile + local 14-joint draft
                    ...
                    └─X─> production runtime gateway / SerialPort

fake transport ──> AethorArmSerialSession
               ├── request-id + boot/session registry
               ├── GET_JPOS/TEL motor-frame projector
               └── latest-only pull channel ──> (production event pump not registered)
...
- Aethor 的 `AethorArmMotorFrameV1` 是 adapter 到 UI 的信任边界，不是第二个串口入口。Schema 保留无序子集和异常证据，并以可选 `identityConflict` / `unexpectedMotorIds` 表达固件 mask 中无法用普通角度样本描述的身份冲突和范围外 ID；`ingestAethorTwinMotorFrame` 先按左右臂各保留一条最新待处理帧，在 20 ms 提交窗口中把双臂原子写入一次 Zustand，模型提交上限为 50 Hz。入口拒绝同一会话内的 controller/arm 身份切换、旧序号、旧 boot 回流和不兼容帧；Profile 切换/会话重置后才接受新身份。
...
| 动作文档当前编辑、选择和预览标记 | `useActionProgramStore` ephemeral state | 当前编辑会话；有效文档由页面 350 ms 防抖自动提交到动作库，选择和预览标记不持久化 |
| 页面、筛选、选中信号 | URL | 可分享导航状态 |
| 工具窗布局、显示偏好 | Versioned local storage | 本机用户 |
| 静态展示采集 | StaticShowcaseSource | 仅展示，永不产生在线状态 |
| 已保存动作程序 | `useActionProgramStore` versioned local storage | 本机浏览器；有效编辑自动写入，启动时按 Schema 重验；导出文件是备份/交接边界 |
...
- engineering direct 不是浏览器串口旁路：C# 继续独占 transport、规范化单行 ASCII、执行白名单、有限六轴/63 字符固件帧和状态校验，并把真实 TX/RX 写入协议证据；它不应用 supervised 的旧 Profile 关节范围。HTTP 受理产生 `queued + gatewayAccepted`；物理 writer 成功后发布 `sent + transportWritten`。请求不等待 FIFO、`ok` 或到位，迟到回包只进入有界日志；终端可连续提交，队列过期、淘汰、断开和写失败分别收束。唯一后台 reader 继续尝试 `#GETJPOS`，运动期间查询超时只把反馈降为 stale，不自动断开或阻止下一次人工目标。
...
Application 的 `SerialDuplexScheduler` 已在 A1-U2 接入 Dummy 生产网关：持续 RX reader、有界优先级 TX writer、RX 背压、P0 安全预留/低优先级淘汰、P1/P2/P3 公平调度、排队时效和关闭 transport 解锁均由同一运行时负责。协议 parser 与 response correlation 留在各自 adapter。只有 adapter 明确标记的幂等只读 Dummy 查询可在一次 `TimeoutException` 或 Windows 原生错误 121 后延迟 100 ms 重试一次；动作和状态改变写入不具备该标记，任何失败都直接进入原有 fault/释放路径。probe 单独统计 retry，不能把重试解释为动作成功。Dummy 仍以单一 response fence 匹配无标签回包；A1-H1-S 的未注册 `AethorArmSerialSession` 则以会话内严格递增 request ID 允许乱序并发 RSP/ERR，并验证 HELLO/boot/session。GET_JPOS 与 TEL 共用 ID/mask projector；容量为一的 pull 槽丢旧保新，未来唯一事件泵主动取帧，慢或失效下游既不反压 parser，也不进入串口 dispose 链。它只在 fake transport 测试中构造，生产 DI、心跳、REST/SignalR 与真实串口仍不存在。
...
- `shared/contracts/action-program-v1.schema.json` 是文件格式权威源；前端 Zod 在导入、自动保存、导出和本机恢复时重复校验 profile、六轴长度、有限数、模式、来源与 UTC 时间。动作文档不拥有 Profile/URDF 范围约束；`#GETJPOS` 实测点从采集、保存、预览到 6B-S command port 始终保持原值。真实网关可以明确拒绝当次命令，但不能在文档链路静默裁剪。
- 当前编辑与已保存 revision 分离，页面在编辑停止 350 ms 后自动校验并写入版本化 local storage；页面卸载会尝试同步刷新，不再弹出未保存离开确认，删除点位也直接进入同一保存链。库限制为 64 个文档和 4 MiB，未知版本、损坏记录与超限写入失败关闭，不静默迁移。
- 动作文档保存程序级 `speedDegS` 和 `loopEnabled`；新建默认 20 deg/s、循环关闭，旧 V1 文件缺字段时按相同默认值规范化。engineering 运行时消费两项设置；循环由 C# 持续到操作者停止。
...
- 预览只调用 Dummy `loadActionPreview` 写入本地目标草稿，不调用网关，也不改写 Aethor_robo 14 轴控制台草稿。运行则创建 `ActionProgramRunStartRequestV1` 深快照；前端不拥有点位循环、等待计时或串口队列。
- C# `EngineeringActionProgramRuntime` 是 engineering 动作的唯一生命周期 owner。它与 `RobotGateway` 共同保证一个运行快照独占非查询串口命令，逐点等待 `sent + transportWritten`，按“最大设备角差 / speed + 附加等待”推进，单次结束为 `finishedUnconfirmed`。循环、操作员停止、终端停止、结构化停止、断开和 dispose 都在该 owner 内收束；停止会取消未来点位并尝试 `!STOP`、`!DISABLE`。
- REST 提供运行快照、开始和停止，SignalR 只发布 `actionProgramRunSnapshot` 变化通知。前端按 session 和 `updatedAtUtc` 拒绝旧事件，session identity 改变时清空运行态。运行完成不表示到位，停止完成也不表示设备确认去使能。
- 原 `ActionProgramRunner` 保留为反馈确认式监督内核，只依赖未注册的 `IActionProgramCommandPort`，要求 `completed + feedbackConfirmed` 并支持 checkpoint。它没有生产 adapter，不能与 engineering 的人工完成模型混用。
