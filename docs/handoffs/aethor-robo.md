...
| 项 | 值 |
|---|---|
| Track | A1-U1 串口双工软件门与双 Profile 终端 |
| 状态 | `DONE` |
...
| 当前阶段交接 | [aethor-robo-a1-u1.md](aethor-robo-a1-u1.md) |
...
- A1-U1 已把该并发设计落为未注册生产 DI 的 `SerialDuplexScheduler` 软件门，并将 `/terminal` 改成双 Profile 外壳；Aethor 分支只做候选模板校验，发送仍固定禁用。完整证据和迁移边界见独立 A1-U1 交接。
...
- 没有 Aethor C# codec、pending request registry、REST/SignalR 投影或真实串口入口。共用持续 RX/优先 TX 调度器仅是未接线的软件门，不能算作 Aethor adapter。
...
- 串口终端外壳已支持 Aethor 候选校验；真实 TX/RX、连接和设备状态仍只属于 Dummy runtime。Dummy 自身尚未迁移到新双工调度器。
...
3. 复用已验证的双工调度基础设施，实现独立 C# Aethor codec 与 pending request registry；不得把未接线软件门或 Dummy codec 当成生产 adapter。
